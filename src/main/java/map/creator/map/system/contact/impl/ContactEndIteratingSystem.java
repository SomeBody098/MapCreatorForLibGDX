 ...
 * Only supports endContact method!
 */
public abstract class ContactEndIteratingSystem extends ContactIteratingSystem {

    public ContactEndIteratingSystem() {
        super();
    }

    public ContactEndIteratingSystem(ObjectEntityFilter filter) {
        super(filter);
    }

    /**
     * The method determines the type of contact, as well as performs special actions if the result of the method <b>endContact</b> was true.
    ...
    protected void processEntity(Entity entity, float deltaTime) {
        ...
        if (Objects.requireNonNull(typeComponent.type) == ContactType.END && endContact(dataComponent, deltaTime)) {
    ...
    public boolean beginContact(ContactDataComponent component, float deltaTime) {
        throw new UnexpectedBehaviorException("Mustn't call beginContact in ContactEndIteratingSystem!");
    }

    @Override
    public boolean stayContact(ContactDataComponent component, float deltaTime) {
        throw new UnexpectedBehaviorException("Mustn't call stayContact in ContactEndIteratingSystem!");
    }
